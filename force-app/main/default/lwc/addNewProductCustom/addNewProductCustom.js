...
import findProducts from '@salesforce/apex/customAddProductController.findProducts';
import saveProducts from '@salesforce/apex/customAddProductController.saveProducts';
import getproductfamily from '@salesforce/apex/customAddProductController.getproductfamily';
...
export default class AddNewProductCustom extends LightningElement {
    ...
    openModal() {
        this.isModalOpen = true;
        // Reset filter form when opening modal
        this.FilterForm = {"ProductFamily" : "", "ProductCode" : ""};
        findProducts({recordId : this.recordId, name : this.searchKey , productCode : '' , productFamily : [], RecordLimit : 5 })
        .then(result => {
            console.log(result);
            let dataObj = JSON.parse(result);
            console.log(dataObj);
    ...
    ApplyFilter(){
        ...
        if(searchBox.value == '' || searchBox.value == undefined){
            ...
            this.isFirstPage  = false;
            // Pass the Product Code filter to the Apex method
            const productCodeFilter = this.FilterForm["ProductCode"] || '';
            findProducts({recordId : this.recordId, name : this.searchKey , productCode : productCodeFilter , productFamily : this.FilterForm["ProductFamily"] || [], RecordLimit : 1000 }).then(result => {
                let dataObj = JSON.parse(result);
                console.log(dataObj);
                this.ShowTableData = dataObj.productList;
                this.filteredData = dataObj.productList;
                ...
                this.RecalculateselectedProductCode();
                console.log('filteredProductData = ', this.ShowTableData);
            }).catch(error => {
    ...
    clearFilter(){
        this.datafilter();
        this.FilterForm = {"ProductFamily" : "", "ProductCode" : ""};
        // Reset the Product Code input field
        const productCodeInput = this.template.querySelector('lightning-input[name="ProductCode"]');
        if (productCodeInput) {
