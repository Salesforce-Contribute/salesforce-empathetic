    ...
    async handleSave(){
        ...
        if(toBeupdated.length > 0 ) {
            try{
                ...
                const result = await updateAmount({recordId:toBeupdated[0].Id, data:toBeupdated});
                if(result=='success'){
                    let notify = await notifyRecordUpdateAvailable([{recordId:toBeupdated[0].Id}])
                    console.log('noteify: '+ notify);
                }
